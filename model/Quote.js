...
class Quote {
  static async generateQuote(mood = "mindfulness") {
    try {
      ...
      const findQuote = await getDB()
        .collection("Quotes")
        .findOne({ quote: quoteResponse });
      ...
      const fileVoiceName = `${quoteResponse.split(" ")[0].replace('"', "")}-${
        quoteResponse.split(" ")[1]
      }-${quoteResponse.split(" ")[2]}-${new Date().getTime()}.mp3`;
      const pathVoiceName = path.join(
        __dirname + "/../public/music/" + fileVoiceName
      );
