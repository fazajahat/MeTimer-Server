...
class AI {
    ...
    static async journalResponse(journalContent) {
        const prompt = `give me a 1 sentence motivational human response about this journal "${journalContent}".`;
    ...
    static async responseChatAI(chat, id) {
        const prompt = `I want you to act a psychologist. i will provide you my thoughts. i want you to give me scientific suggestions that will make me feel better.  i want you to give response maximum 1 sentences. my thought "${chat}"`;
        const response = await openai.completions.create({
            model: "text-davinci-003",
            prompt: prompt,
            max_tokens: 2048,
